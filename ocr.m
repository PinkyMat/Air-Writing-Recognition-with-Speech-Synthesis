...
if ischar(userLanguage)
    ...
    if ~isCustomLanguage        
        ...
        lang = modifiedLang;           
        coder.internal.errorIf(~logical(exist(lang,'file')),...
...
if isempty(p)   
    ...
    p = inputParser();      
    addOptional(p, 'ROI', []);
    addParamValue(p, 'TextLayout',   defaults.TextLayout);
    addParamValue(p, 'Language',     defaults.Language);
    addParamValue(p, 'CharacterSet', defaults.CharacterSet);
    addParamValue(p, 'PreprocessBinaryImage', ...
